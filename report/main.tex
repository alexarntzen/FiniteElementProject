...
The solutions are compaired to the analytic solution. The maximal error decreases with the number of degrees freedom and indicates that the metod is convergent. 
\end{abstract}
...
The finite dimetional problem is then solved numerically for two simple cases. The result is compared to the corresponding analytic solution. 
...
\end{equation*}
\begin{remark}
Note that when $\partial \Omega_{D} = \partial \Omega$ we have the homogenous Dirichlet boundary conditions. Also by \cite[p. 39]{AQuart} when $\partial \Omega_D = \emptyset$ the poission problem does not have a unique solution. In the rest of the text we assume $\partial \Omega_D \neq \emptyset$ 
\end{remark}
...
\end{proof}
\noindent Then $u$ also reveals the corresponding solution with neumann bundary conditions. 
...
\end{proposition}
\begin{proof}
    This proof is heavely influenced by \cite[p. 44-45]{AQuart}.
...
By discretizing $\Omega$ into $M$ triangles $K_k$, , as seen in figure \ref{plot-mehses}, defined by corner nodes $(x_i,y_i)$, we have basis functions corresponding to each node.
Let 
\begin{equation*}
...
The Poisson problem stated in \eqref{poission-problem} will have infinitely many solutions $u$ when boundary conditions are not considered \cite[p. 39]{AQuart}.
...
As stated in \eqref{poission-problem} we have homogenous Dirichlet conditions on parts of the domain; $u(x,y) = 0, \forall (x,y) \in \partial \Omega_D$.
This has not been considered in the given construction of $\bm A,\bm f$, which could be denoted as a proto problem.
Hence, to find the solution $u \in X_h$ we will use the so called Big Number approach to enforce the Dirichlet boundary conditions.

For nodes on the boundary $(x_i,y_i), i=n+1,\cdots\tilde n$ we set
...
Examples of such a mesh for $N = 16,64,256$ could be seen in figure \ref{plot-meshes}.
...
Numerical experiments with increasing degrees of freedom results in lower relative maximal error, as seen in figure \ref{convergence}. The numerical solution for $N = 1024$ nodes is plotted in figure \ref{solution-error}
...
\begin{figure*}
    ...
    \begin{subfigure}[b]{0.5\textwidth}
        ...
        \caption{Relative error of numerical solutios to the poission problem with Dirichlet boundary conditions for multiple mesh discs. The number of nodes given by $N_n = \{ 2^n\}_{n=4}^{12}$
